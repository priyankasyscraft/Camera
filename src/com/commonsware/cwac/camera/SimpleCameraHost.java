...
public class SimpleCameraHost implements CameraHost {
	...
	public void configureRecorderOutput(int cameraId, MediaRecorder recorder) {
		...
		File file = new File(filepath, "VideoTemp" + ".mp4");
		if (file.exists()) {
			file.delete();
		}
	...
	public void configureRecorderProfile(int cameraId, MediaRecorder recorder) {
		...
		if (CamcorderProfile.hasProfile(CamcorderProfile.QUALITY_CIF)) {
			Log.d("QUALITY_CIF------------", "QUALITY_CIF");
			recorder.setProfile(CamcorderProfile.get(cameraId,
					CamcorderProfile.QUALITY_CIF));// QUALITY_CIF - (352 x 288)
													// resolution
		} else if (CamcorderProfile.hasProfile(CamcorderProfile.QUALITY_QVGA)) {
			Log.d("QUALITY_QVGA-------------", "QUALITY_QVGA");
			recorder.setProfile(CamcorderProfile.get(cameraId,
					CamcorderProfile.QUALITY_QVGA));// QUALITY_QVGA - (320x240)
													// resolution.
		} else if (CamcorderProfile.hasProfile(CamcorderProfile.QUALITY_HIGH)) {
			Log.d("QUALITY_HIGH-----------------", "QUALITY_HIGH");
			recorder.setProfile(CamcorderProfile.get(cameraId,
					CamcorderProfile.QUALITY_HIGH));// QUALITY_HIGH - (720 x
													// 480) resolution.
		} else {
			Log.d("else ------------QUALITY_HIGH", "QUALITY_HIGH");
			recorder.setProfile(CamcorderProfile.get(cameraId,
					CamcorderProfile.QUALITY_CIF));// QUALITY_CIF - (352 x 288)
													// resolution
		}
	...
	// "Video.mp4");
	// // return (new File(getVideoDirectory().getAbsolutePath()));
	// return file;
	// }
	...
	// // String ts=
	// // new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(new
	// // Date());
	// return ("Video.mp4");
	// }
